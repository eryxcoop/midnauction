      ...
      <Box>
        ...
        <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: '1fr 1fr' }, gap: 2 }}>
          <Box>
            ...
            <Chip 
              label={`Round ${auction.currentRound}/${auction.rounds} - ${auction.phase}`} 
              color={auction.phase === 'commitment' ? 'primary' : auction.phase === 'revealing' ? 'warning' : 'success'}
              size="small"
            />
          </Box>
          <Box>
            ...
            <Typography variant="body1">{auction.secretBids}</Typography>
          </Box>
        </Box>
      </Box>
          ...
          <Box sx={{ display: 'grid', gridTemplateColumns: '1fr', gap: 1 }}>
            <Box>
              ...
              <Typography variant="body1" sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                {auction.currentRound} / {auction.rounds}
              </Typography>
            </Box>
            <Box>
              <Typography variant="body2" color="text.secondary">
          ...
          <Box sx={{ display: 'grid', gridTemplateColumns: '1fr', gap: 1 }}>
            <Box>
              ...
              <Typography variant="body1" sx={{ fontFamily: 'monospace', fontSize: '0.8rem' }}>
                {auction.auctioneerPK.substring(0, 20)}...
              </Typography>
            </Box>
          </Box>
        </Box>
              ...
              {auction.revealedBids.map((bid, index) => (
                <Box key={index} sx={{ p: 1, border: '1px solid', borderColor: 'divider', borderRadius: 1 }}>
                  <Typography variant="body2">
                    Bid #{index + 1}: {(Number(bid.value) / 100).toFixed(2)} USD
                  </Typography>
                </Box>
              ))}
            </Box>
